...
function updateNavButtons() {
  if (state.reviewMode) {
    els.prev.hidden = state.reviewQueue.length === 0;
    els.prev.disabled = state.reviewQueue.length === 0 || state.reviewIdx <= 0;
  } else {
    els.prev.hidden = state.questions.length === 0;
    els.prev.disabled = state.questions.length === 0 || state.idx <= 0;
  }
  updateReviewBadge();
}
...
function enterReviewMode() {
  ...
  state.reviewIdx = 0;
  els.modeLabel.textContent = state.lang === 'en' ? 'Review Mode' : 'وضع المراجعة';
...
function prevQuestion() {
  if (state.reviewMode) {
    if (!Array.isArray(state.reviewQueue) || state.reviewQueue.length === 0) return;
    state.reviewIdx = Math.max(0, state.reviewIdx - 1);
    renderReviewQuestion();
  } else {
